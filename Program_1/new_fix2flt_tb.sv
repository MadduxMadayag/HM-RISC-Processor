...
module new_int2flt_tb();
  ...
  initial begin				 // test sequence
    ...
    $monitor("data_mem.core0, 1 = %b  %b %t",f0.data_mem1.mem_core[3],f1.DM1.mem_core[3],$time);

    //#20ns reset = '0;
    disp2(int_in);			 // subroutine call
	int_in = 16'h8000;			 // minimum nonzero positive = 1/128
	disp2(int_in);
...
task automatic disp2(input logic [15:0] int_in);
	...
	f1.DM1.mem_core[1] = int_in[15:8];   // load operands into your memory
	f1.DM1.mem_core[0] = int_in[ 7:0];
	...
	wait(ack0);
	#10ns;
  flt_out  = {f1.DM1.mem_core[3],f1.DM1.mem_core[2]};	 // results from your memory
  ...
  $display("IN=0x%h,  DUT=0x%b, REF=0x%b, MATH=0x%b",
			int_in, flt_out, flt_out0, flt_outM);
	// Compare DUT vs. reference DUT
    if (flt_out === flt_out0) 
      score0++;
    else 
      $display("Mismatch DUT vs REF: DUT=0x%b REF=0x%b", flt_out, flt_out0);

    // Compare DUT vs. math model
    if (flt_out === flt_outM) 
      scoreM++;
    else 
      $display("Mismatch DUT vs MATH: DUT=0x%b MATH=0x%b", flt_out, flt_outM);
